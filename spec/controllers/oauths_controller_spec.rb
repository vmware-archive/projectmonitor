...
<feed>
  <author><name>First Last</name><email>email@example.com</email></author>
    ...
    it "should redirect to login path when not in authorized domain list" do
      sample_user_response = <<-eos
<?xml version='1.0' encoding='UTF-8'?>
<feed>
  <author><name>First Last</name><email>email@unauthorized.com</email></author>
</feed>
      eos
      access_token = mock(:token => "accesstoken", :secret => "accesssecret", :get => mock(:body => sample_user_response))
      request_token = mock
      ...
      flash[:notice].first.should == "Email not in authorized domains."
    end
