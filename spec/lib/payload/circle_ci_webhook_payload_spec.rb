...
describe CircleCiWebhookPayload do

  let(:fixture_file)      { "success.json" }
  let(:fixture_content)   { load_fixture('circleci_webhook_examples', fixture_file) }
  let(:payload)           { CircleCiWebhookPayload.new.tap{|p| p.status_content = fixture_content} }
