...
describe Project do
  ...
  describe '#record_failure!' do
    let(:project) { FactoryGirl.build(:project, enabled: true, failures: 0) }
    context 'when the project is enabled' do

      context 'when there are fewer than 3 failures' do
        before do
          project.failures = 2
        end

        it 'should increment the number of failures' do
          project.record_failure!
          project.failures.should == 3
        end

        it 'should return false' do
          project.record_failure!.should == false
        end

      end

      context 'when there have been 3 or more failures' do
        before do
          project.failures = 3
        end

        it 'should reset the failures back to 0' do
          project.record_failure!
          project.failures.should == 0
        end

        it 'should disable the project' do
          project.record_failure!
          project.enabled.should == false
        end

        it 'should return true' do
          project.record_failure!.should == true
        end

      end
    end
    context 'when the project is not enabled' do

      it 'should not do anything' do
        project.enabled = false
        project.should_not_receive(:enabled=)
        project.should_not_receive(:failures=)
        project.record_failure!
      end
